...
import shutil
import os
from api.engine_bioacustico import procesar_muestras_acusticas, generar_grafico_clinico
from api.reportes_pdf import generar_pdf_clinico
...
async def analizar_y_reportar(
    ...
    # 1. Cálculo acústico determinístico
    metricas = procesar_muestras_acusticas(tmp_vocal)
    
    # 2. Generación del gráfico dual (DDF + Espectrograma)
    img_path = "/tmp/graficos_clinicos.png"
    generar_grafico_clinico(metricas, img_path)
    
    # 3. Síntesis diagnóstica con Groq API (Llama 3.3 70B)
    ...
    if groq_key:
        try:
            ...
            prompt = (
                f"Actúa como Fonoaudiólogo especialista en voz en Argentina. Redacta una síntesis diagnóstica fonoaudiológica formal y concisa "
                f"para el paciente {nombre} ({edad} años, {sexo}). Métricas objetivas obtenidas: F0={metricas['f0_mean']} Hz, "
                ...
                f"Usa terminología clínica formal en español rioplatense sin inventar datos."
            )
            ...
            sintesis_ia = f"Síntesis automática: AVQI = {metricas['avqi']}. F0 = {metricas['f0_mean']} Hz."

    # 4. Compilación del PDF clínico formal
    pdf_path = "/tmp/informe_clinico.pdf"
    paciente_dict = {
        "nombre": nombre, "dni": dni, "edad": edad, "sexo": sexo,
        "motivo": motivo, "derivador": derivador, "sintesis_ia": sintesis_ia, "tmf": tmf
    }
