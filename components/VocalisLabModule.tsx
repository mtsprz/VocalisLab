...
export default function VocalisLabModule() {
  ...
  const ejecutarAnalisisYDescarga = async () => {
    if (!blobVocal) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8">
      <div className="max-w-6xl mx-auto mb-8 flex flex-col md:flex-row items-start md:items-center justify-between border-b border-slate-800 pb-6 gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">
                Plataforma de Bioacustica Fonoaudiologica & AVQI v03.01
              </p>
            </div>
          </div>
        </div>

        <div className="flex items-center gap-2 bg-slate-900/90 border border-slate-800 px-4 py-2 rounded-xl backdrop-blur-md">
          ...
          >
            {dispositivos.map((d) => (
              <option key={d.deviceId} value={d.deviceId} className="bg-slate-900 text-white">
                {d.label || `Entrada de audio ${d.deviceId.slice(0, 5)}`}
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl p-5 rounded-2xl shadow-xl space-y-4">
            <div className="flex items-center gap-2 text-sky-400 font-semibold text-sm">
              ...
              <span>Evaluacion Perceptivo-Auditiva (0: Normal, 3: Severo)</span>
            </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl p-5 rounded-2xl shadow-xl flex flex-col items-center">
            <div className="w-full flex items-center justify-between mb-3 text-xs text-slate-400">
              <span className="flex items-center gap-1.5 font-medium">
                ...
                Senal en Vivo (Osciloscopio)
              </span>
              {(grabandoVocal || grabandoHabla) && (
                <span className="flex items-center gap-1 text-red-400 animate-pulse font-bold">
                  ● GRABANDO 44.1 kHz
                </span>
              )}
            </div>
            <div className="w-full h-28 bg-slate-950 rounded-xl overflow-hidden border border-slate-800 shadow-inner relative flex items-center justify-center">
              ...
              {!grabandoVocal && !grabandoHabla && (
                <span className="absolute text-xs text-slate-600 font-medium">
                  Presiona grabar para capturar la senal
                </span>
              )}
            </div>
          </div>
