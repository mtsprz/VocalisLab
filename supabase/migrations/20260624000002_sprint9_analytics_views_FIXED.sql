-- Sprint 9: Dashboard Analytics (DEFENSIVO - sin tablas faltantes)
-- Solo usa tablas que existen: patients, sessions, appointments, profiles,
-- clinical_history_records, nba_decisions, home_guides, distribution_logs
...
-- ============================================
-- 8. TESTS POR AREA (placeholder - tabla no existe aún)
-- ============================================
CREATE OR REPLACE VIEW v_tests_by_area AS
SELECT
  'Sin datos'::text AS area,
  0::bigint AS total,
  0::numeric AS avg_percentage,
...
SELECT 'session_completed' AS event_type,
  'Sesion con ' || coalesce(p.name, 'paciente') AS description, s.created_at
FROM sessions s
...
SELECT 'guide_sent' AS event_type,
  coalesce(title, 'Guia') AS description, sent_at
FROM home_guides
